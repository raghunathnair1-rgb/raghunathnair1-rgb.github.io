...
# --- coverage-gate snapshot (blog-logic 100% floor) for the factory tab ---
python3 /home/raghu/harness/gen_coverage.py >/dev/null 2>&1 || echo "📊 coverage.json skipped"
# --- sun-arc astronomy proxied from wttr.in (server-side; avoids per-visitor rate limits) ---
python3 /home/raghu/harness/gen_sun.py >/dev/null 2>&1 || echo "🌅 sun.json skipped"
...
if command -v claude >/dev/null 2>&1; then
  ...
  DIFF=$(git --no-pager diff HEAD -- . ':(exclude)status.json' ':(exclude)spark.json' ':(exclude)router.json' ':(exclude)deploy.json' ':(exclude)activity.json' ':(exclude)watchdog.json' ':(exclude)coverage.json' ':(exclude)ideas.json' ':(exclude)sun.json' 2>/dev/null | head -c 10000 || true)
  if [ -n "$DIFF" ]; then
